...
function processNextStep(name, nextStep, payload, callback)
{
  var type;
  ...
  if (!nextStep)
  {
    this.logger.debug({message: 'no further steps defined, stop right here', name: name, payload: payload});
  ...
  if (typeof payload != 'object')
  {
    this.logger.error({message: 'payload is not an Object', name: name, nextStep: nextStep, payload: payload});
    callback(new Error('payload from <', name, '> for <', nextStep, '> is not an Object'));
    return;
  }
  ...
  // 1:many
  else if (Array.isArray(nextStep) && (type = nextStep.find(function(el){ return el in payload; })))
  {
    // handle it as `name.type`, example: `message.attachments`
    // keep original payload, since `type` is just a marker within the branch
    handleBranch.call(this, type, payload, callback);
  }
  ...
  else if (typeof nextStep == 'object')
  {
    ...
    asynckit.parallel(nextStep, function(types, prop, cb)
    {
      ...
      {
        handleBranch.call(this, prop, propType, payload[name], done);
      }.bind(this), cb);
