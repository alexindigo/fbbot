 ...
 * Note: `type` or `data` should be provided
 *
...
function send(user, type, data, callback)
{
  ...
  // TODO: Get only `id` or `phone_number` for the object, since it might have other things like `first_name`, `last_name`, etc
  // assemble message
...
function generateMessage(type, data)
{
  ...
  switch (type)
  {
    // default message type, no need to perform special actions
    // will be sent as is
    case types.PAYLOAD:
      message = data;
      break;
